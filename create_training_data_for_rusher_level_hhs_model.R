...
# Now, this flattens into a matrix with the 6 relevant columns for each player we consider
flat_features_matrix = model_training_with_values %>% select(x_diff, y_diff, o_diff, dir_diff, a_diff, s_diff) %>% as.matrix()
flat_features_matrix[is.na(flat_features_matrix)] = 0

flat_features = matrix(t(flat_features_matrix), ncol=(1 + 2*NUM_PLAYERS_TO_CONSIDER) * 6, byrow=TRUE)
